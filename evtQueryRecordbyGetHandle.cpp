...
void PrintRecord(PEVTLOGRECORD currentRecordPtr)
{
	printf("-------------------------------\n");
	printf("Length:%d\n", currentRecordPtr->Length);
	printf("Reserved:%d\n", currentRecordPtr->Reserved);
	printf("RecordNumber:%d\n", currentRecordPtr->RecordNumber);
	struct tm LocalTime;
	char LocalBuf[26];
	__int64 CalTime = (__int64)currentRecordPtr->TimeGenerated;
	_localtime64_s(&LocalTime, &CalTime);
	strftime(LocalBuf, 26, "%Y-%m-%d %H:%M:%S", &LocalTime);
	printf("TimeGenerated:%s\n", LocalBuf);

	memset(LocalBuf, 0, 26);
	CalTime = (__int64)currentRecordPtr->TimeWritten;
	_localtime64_s(&LocalTime, &CalTime);
	strftime(LocalBuf, 26, "%Y-%m-%d %H:%M:%S", &LocalTime);
	printf("TimeWritten:%s\n", LocalBuf);
	printf("EventID:%d\n", currentRecordPtr->EventID);
	printf("EventType:%d\n", currentRecordPtr->EventType);
	printf("NumStrings:%d\n", currentRecordPtr->NumStrings);
	printf("EventCategory:%d\n", currentRecordPtr->EventCategory);
	printf("ReservedFlags:%d\n", currentRecordPtr->ReservedFlags);
	printf("ClosingRecordNumber:%d\n", currentRecordPtr->ClosingRecordNumber);
	printf("StringOffset:%d\n", currentRecordPtr->StringOffset);
	printf("Length:%d\n", currentRecordPtr->Length);
	printf("UserSidLength:%d\n", currentRecordPtr->UserSidLength);
	printf("UserSidOffset:%d\n", currentRecordPtr->UserSidOffset);
	printf("DataLength:%d\n", currentRecordPtr->DataLength);
	printf("DataOffset:%d\n", currentRecordPtr->DataOffset);

}

PVOID QueryEvent(PVOID mapAddress, int len)
{
...
BOOL QueryEventMain(HANDLE fileHandle)
{
	...
	mapAddress = QueryEvent(mapAddress, len);
...
int main(int argc, char *argv[])
{
	if (argc != 2)
	{
		printf("\nQuery each records of the Windows Event Viewer Log (EVT) files\n");
		printf("Support:WinXP and later\n");
		printf("Usage:\n");
		printf("     %s <absolute or relative file path>\n", argv[0]);
		printf("eg:\n");
		printf("     %s sysevent.evt\n", argv[0]);
		printf("     %s system.evtx\n", argv[0]);
